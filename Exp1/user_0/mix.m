...
lower =  [repmat(pars_lower,4,1)];
upper =  [repmat(pars_upper,4,1)];

%%
% Retreiving data
load('/home/aghambaryan/myNCMtask/data_dom2val');
subj = isubject;

for x = 1:length(subj)
        % Saving the current subject's data
        tmp = data.subject(subj(x));    
        save('tmp.mat','tmp')
